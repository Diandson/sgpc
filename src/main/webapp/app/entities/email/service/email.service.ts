...
type RestOf<T extends IEmail | NewEmail> = Omit<T, 'dateEnvoi'> & {
...
export class EmailService {
  ...
  protected convertDateFromClient<T extends IEmail | NewEmail | PartialUpdateEmail>(email: T): RestOf<T> {
    return {
      ...email,
      dateEnvoi: email.dateEnvoi?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEmail: RestEmail): IEmail {
    return {
      ...restEmail,
      dateEnvoi: restEmail.dateEnvoi ? dayjs(restEmail.dateEnvoi) : undefined,
    };
  }
